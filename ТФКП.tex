...
\begin{document}
	...
	\begin{definition}
		Пусть $w = f(z) : D_z \to D_\omega$. Точка $z_0 \in D_z$ называется неподвижной точкой при отображении $\omega = f(z)$, если $f(z_0) = z_0$.
	\end{definition}
	
	\begin{theorem}
		Если дробно-линейное отображение имеет более двух неподвижных точек, то оно имеет вид $w = z$.
	\end{theorem}
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			\item $c = 0$, $a \neq d$:
			\[z_1 = \frac{b}{d-a} \text{ --- неподвижная точка}\]
			\[z_2 = \infty \text{ --- неподвижная точка}\]
			
			\item $c \neq 0$, $z_1$, $z_2$ --- корни уравнения (*):
			$z_1$, $z_2$ --- неподвижные точки.\\
			Одна $(z_1 = z_2)$ или две $(z_1 \neq z_2)$ неподвижные точки.
			
			\item $c = 0$, $a = d$, $b = 0$:
			\[\forall z \in \overline{\mathbb{C}} \text{ --- неподвижная точка}\]
			\[\omega = z\]
		\end{enumerate}
	\end{proof}
	
	\begin{theorem}
		$\exists !$ дробно-линейное отображение, переводящее три различные точки $z_1, z_2, z_3 \in \overline{\mathbb{C}}$ в три различные точки $\omega_1, \omega_2, \omega_3 \in \overline{\mathbb{C}}$
	\end{theorem}
	%%%%%%%%%%%%%%%%%%%%%%%PICTURE ONE%%%%%%%%%%%%%%%%%%%%%
	\begin{proof}
		1) Существование:
		\begin{align*}
			&z_j \to \omega_j,\; j = 1, 2, 3 \\
			&\xi = \frac{az+b}{cz+d} \quad : \quad z_1 \to 0,\; z_2 \to \infty,\; z_3 \to 1 \\
			&(1) \implies az_1 + b = 0 \implies b = -az_1 \\
			&(2) \implies cz_2 + d = 0 \implies d = -cz_2 \\
			&\xi = \frac{a(z-z_1)}{c(z-z_2)} = K \cdot \frac{z-z_1}{z-z_2}\\
			&(3) \implies K \cdot \frac{z_3 - z_1}{z_3 - z_2} = 1 \implies K = \frac{z_3 - z_2}{z_3 - z_1}\\
			&\xi = \frac{z - z_1}{z - z_2} \cdot \frac{z_3 - z_2}{z_3 - z_1} \\
			&\xi = \frac{\omega - \omega_1}{\omega - \omega_2} \cdot \frac{\omega_3 - \omega_2}{\omega_3 - \omega_1} \\
			&h(\omega) \quad : \quad \omega_1 \to 0,\; \omega_2 \to \infty,\; \omega_3 \to 1 \\
			&f(z) = h^{-1}(g(z)) \quad \omega = h^{-1}(g(z)) \text{ --- дробно-линейное отображение} \\ 
			&z_j \to \omega_j \\
			&h(\omega) = g(z) \implies \frac{\omega - \omega_1}{\omega - \omega_2} \cdot \frac{\omega_3 - \omega_2}{\omega_3 - \omega_1} = \frac{z - z_1}{z - z_2} \cdot \frac{z_3 - z_2}{z_3 - z_1}
		\end{align*}
		...
		От противного:
		\begin{align*}
			&\text{Пусть } \exists \overline{f} \text{ --- дробно-линейное отображение} \quad : \quad z_j \xrightarrow{\overline{f}} \omega_j,\; j = 1, 2, 3 \\
			&\overline{f}(z) \neq f(z) \\
			&\psi(z) = \overline{f}^{-1}(f(z)) \text{ --- дробно-линейное отображение} \\ 
			&z_j \xrightarrow{\psi} z_j \implies \psi(z) \text{ имеет три различных неподвижных точки} \implies \psi(z) = z \implies \overline{f}^{-1}(f(z)) = z \implies f(z) = \overline{f}(z) \\
			&\text{Получили противоречие}
		\end{align*}
	\end{proof}
	...
	\begin{definition}
		Окружность или прямую будем называть обобщенной окружностью.
	\end{definition}
	
	\begin{theorem}
		$\forall$ дробно-линейное отображение переводит обобщенную окружность в обобщенную окружность.
	\end{theorem}
	
	\begin{proof}
		1) Для первых трех базовых отображений утверждение очевидно. \\
		2) Покажем, что теорема верна для отображения $\omega = \frac{1}{z}$:
		Запишем уравнение обобщенной окружности:
		\[A(x^2 + y^2) + Bx + Cy + D = 0, \quad A, B, C, D \in \R\]
		\[x^2 + y^2 = z \overline{z}\]
		\[\begin{cases} 
			z = x + iy\\
			\overline{z} = x - iy
		\end{cases}
		\implies x = \frac{z + \overline{z}}{2}, \quad y = \frac{z - \overline{z}}{2i}\]
		\[Az \overline{z} + \frac{B}{2}(z + \overline{z}) + \frac{C}{2i}(z - \overline{z}) + D = 0\]
		\[A \frac{1}{\omega \overline{\omega}} + \frac{B}{2}\left(\frac{1}{\omega} + \frac{1}{\overline{\omega}}\right) + \frac{C}{2i}\left(\frac{1}{\omega} - \frac{1}{\overline{\omega}}\right) + D = 0 \;|\; \cdot \omega \overline{\omega}\]
		\[D \cdot \omega \overline{\omega} + \frac{B}{2}(\omega + \overline{\omega}) + \frac{C}{2i}(\overline{\omega} - \omega) + A = 0\]
		\[\omega = u + iv, \quad \overline{\omega} = u - iv\]
		\[D \cdot (u^2 + v^2) + B u - C v + A = 0\]
	\end{proof}
	
	\begin{definition}
		$\gamma_{R} = \{ z \in \mathbb{C} : |z - z_0| = R\}$ 
		Точки $z$, $z^{*}$ называются симметричными относительно окружности $\gamma_{R}$, если:
		\begin{enumerate}
			\item они лежат на одном луче, выходящем из центра окружности;
			\item произведение расстояний от этих точек до центра окружности равно $R^2$.
		\end{enumerate}
	\end{definition}
	...
	\begin{remark}
		\begin{align*}
			...
			&2)\; |z - z_0| \cdot |z^{*} - z_0| = R^2 \\
			&z^{*} - z_0 = |z^{*} - z_0| \cdot e^{i \arg(z^{*} - z_0)} \\
			&z^{*} - z_0 = \frac{R^2 \cdot e^{i \arg(z - z_0)}}{|z - z_0|} \\
			&z^{*} = z_0 + \frac{R^2}{|z - z_0| e^{-i \arg(z - z_0)}} \implies z^{*} = z_0 + \frac{R^2}{\overline{z - z_0}}
		\end{align*}
	\end{remark}
	
	\begin{definition}
		Точки $z$ и $z^{*}$ называются симметричными относительно прямой, если они лежат на одном перпендикуляре к прямой на одинаковых расстояниях по разные стороны от прямой.
		%PICTURE 3 and PICTURE 4%
	\end{definition}
	
	\begin{statement}
		Точки $z$ и $z^{*}$ симметричны относительно окружности $\gamma_R$ $\Longleftrightarrow$
		$\forall \Gamma$ (окружность) такой, что $z, z^{*} \in \Gamma$, выполняется $\Gamma \perp \gamma_R$.
		%PICTURE 5
	\end{statement}
	
	\begin{proof}
		1) $z$ и $z^{*}$ симметричны относительно $\gamma_R$:
		\[|z -z_0| \cdot |z^{*} - z_0| = R^2 \implies \text{ длина отрезка касательной равна } R \implies \gamma_R \perp \Gamma\]
		
		2) $\gamma_R \perp \Gamma$:
		%PICTURE 6
		$z$ и $z^{*}$ лежат на одном луче, выходящем из точки $z_0$:
		\[|OB|^2 = |OC| \cdot |OD| \implies R^2 = |z - z_0| \cdot |z^{*} - z_0| \implies z \text{ и } z^{*} \text{ симметричны относительно окружности}\]
	\end{proof}
	...
	\begin{proof}
		$\gamma_z$ --- обобщенная окружность, $\gamma_z \subset \overline{\mathbb{C}}$. \\
		$z$, $z^{*}$ --- точки, симметричные относительно $\gamma_z$. \\
		$\omega = f(z)$ --- дробно-линейное отображение. \\
		$\gamma_z \xrightarrow{f} \gamma_\omega$ --- обобщенная окружность. \\
		$z \xrightarrow{f} \omega$, $z^{*} \xrightarrow{f} \omega^{*}$. \\
		$\Gamma_z$ --- произвольная окружность такая, что $z, z^{*} \in \Gamma_z$ и $\Gamma_z \perp \gamma_z$. \\
		$\Gamma_z \xrightarrow{f} \Gamma_\omega$ --- обобщенная окружность. \\
		$f$ --- конформное отображение $\implies$ по свойству сохранения углов $\gamma_\omega \perp \Gamma_\omega$. \\
		Следовательно, $\omega$ и $\omega^{*}$ симметричны относительно $\gamma_\omega$.
	\end{proof}
	\subsection{Примеры}
	\begin{example}
		Дана область $D_z = \{z \in \Cm : \operatorname{Re} z < 0, \operatorname{Im} z > 0\}$
		\[\omega = \frac{z - i}{z + i}\] 
		\[ \gamma_1 : z = iy, y \geq 0 \]  
		\[ 0 \to -1, \quad i \to 0, \quad \infty \to 1\]
		\[ \gamma_2 : z = x, x \leq 0\] 
		\[ \infty \to 1, \quad -1 \to i,\quad 0 \to -1\]
		%PICTURE 8
		Нужно подставить все три точки справа в уравнение:
		\[|\omega - \omega_0| = R \implies (u - u_0)^2 + (v - v_0)^2 = R^2\]
		Тогда нам удастся найти $u_0, v_0, R$ \\
		Возьмем внутреннюю точку $-1 + i$: $ -1 + i \to \frac{-1}{-1+2i} = \frac{1}{1 - 2i} = \frac{1+2i}{5}$ \\ 
	\textbf{Ответ}: $D_\omega = \{\omega \in \Cm : |\omega| < 1, \operatorname{Im}\omega > 0\}$
	\end{example}
	\begin{example}
		%Picture 9
		Дана область $D_z = \{ z \in \Cm : 0 < \operatorname{Re}z < 1\}$ и $\omega = \frac{z - 1}{z}$
		\[\gamma_1 : z = iy, \quad y \in \R \]
		\[0 \to \infty,\quad  -i \to 1 -i, \quad \infty \to 1\] 
		\[\gamma_2 : z = 1 + iy, \quad y \in \R\]
		\[\infty \to 1, \quad 1 \to 0, \quad 1 + i \to \frac{1 + i}{2}\]
		\[(u - u_0)^2 + (v - v_0)^2 = R^2\]
		\[1: (1 - u_0)^2 + v_0^2 = R^2 \quad (1)\]
		\[0: u_0^2 + v_0^2 = R^2 \quad(2) \]
		\[\frac{1+i}{2} : (\frac{1}{2} - u_0)^2 + (\frac{1}{2} - v_0)^2 = R^2  \quad (3)\]
		Из $(1) - (2)$ получим, что $u_0 = \frac{1}{2}$, затем остается система:
		\[\begin{cases}
			(\frac{1}{2})^2 + v_0^2 = R^2 \quad (1) \\
			(\frac{1}{2} - v_0)^2 = R^2 \quad (3)
		\end{cases} \implies R = \frac{1}{2}, \quad v_0 = 0\] \\
		\textbf{Ответ:} $\{ w \in \Cm : \operatorname{Re} \omega < 1, |\omega - \frac{1}{2}| > \frac{1}{2}\}$
	\end{example}
	\begin{example}
		\textbf{Задача 1 с коллоквиума} \\
		Найти общий вид дробно-линейного отображения, переводящего $D_z = \{ z \in \Cm : \operatorname{Im} z > 0\}$ на $K_\omega = \{ \omega \in \Cm : |\omega| < 1\}$. 
		\begin{solve}
			%picture 10.1
			$a \to 0, \operatorname{Im}a > 0 \quad \overline{a} \to \infty$
			%picture 10.2
			$\omega^{*} = \omega_0 + \frac{R^2}{\overline{\omega - \omega_0}} \text{ после упрощения } \omega^{*} = \frac{1}{\omega}$ \\
			$\omega = \frac{Az + B}{Cz + D} \implies \begin{cases}
				A \cdot a + B = 0 \implies B = -a A\\
				C \cdot \overline{a} + D = 0 \implies D = -\overline{a}C
				\end{cases} \implies \omega = \frac{A(z - a)}{C(z - \overline{a})}$
			Тогда получим $\omega = K \cdot \frac{(z - a)}{z - \overline{a}}$
			Далее нужно определить $K$:
			\[0 \to e^{it} \implies \frac{-a}{-\overline{a}} \cdot K = e^{it} \implies |K| = 1 \text{ или } K = e^{i \alpha}\] 
			\textbf{Ответ:} $\omega = \frac{z - a}{z - \overline{a}} \cdot e^{i \alpha}, \quad \operatorname{Im} a > 0, \alpha \in \R$
		\end{solve}
	\end{example}
	\begin{example}
		\textbf{Задача 2 с коллоквиума} \\
		Найти общий вид дробно-линейного отображения, переводящего $D_z = \{ z \in \Cm : |z| < 1\}$ на $K_\omega = \{\omega \in \Cm : |\omega| < 1\}$. 
		\begin{solve}
			%picture 11.1
			$a \to 0, |a| < 1$
			%picture 11.2
	...
	\begin{example}
		\textbf{Способ 1:}
		$K_z = \{ z \in \Cm : |z - 2| < 1\}$ и $K_\omega = \{\omega \in \Cm : |\omega - ri| < 2\}$ \\
		$\omega(2) = i, \arg \omega(2) = 0$\\
		%picture 12 и a = -\frac{i}{2}
		$\xi = z - 2 \implies \eta = \frac{\omega - 2i}{2}$ 
		\[ \xi = \frac{\eta - a}{1 - \overline{a} \cdot \eta} \cdot e^{i \alpha}\]
		\[ \xi = \frac{\eta + \frac{i}{2}}{1 - \frac{i}{2}\eta} \cdot e^{i \alpha}\]
		\[z - 2 = \frac{\frac{\omega - 2i}{2} + \frac{i}{2}}{1 - \frac{i}{2}(\frac{\omega - 2i}{2})} \cdot e^{i \alpha}\]
		\[z - 2 = \frac{2(\omega - i)}{4 - 2 - i \omega} \cdot e^{i \alpha}\]
		\[z - 2 = \frac{2(\omega - i)}{2 - i \omega} \cdot e ^{i \alpha} \]
		\[z - 2 = \frac{2(\omega(z) - i)}{2 - i \omega(z)} \cdot e^{i \alpha}\]
		\[1 = \frac{2 \cdot \omega'(z) ( 2 - i \omega(z)) + i \omega' (z) \cdot 2 (\omega(z) - i)}{(2 - i \omega(z))^2} \cdot e^{i \alpha}, \text{ где } z = 2, \omega(2) = i\]
		\[ 1 = \frac{\omega'(2)(2 \cdot 3 \cdot e^{i \alpha})}{9}\]
		$\arg \omega'(2) = 0$ и $\omega'(2) = Ae^{-i \alpha}$, причем $A > 0$ \\
		$\arg \omega'(2) = -\alpha, \alpha = 0 \implies \alpha = 0$ \\
		\textbf{Ответ: } $z - 2 = \frac{2(\omega - i)}{2 - i \omega}$ \\ 
		\textbf{Способ 2:}
	\end{example}
	\newpage
\end{document}