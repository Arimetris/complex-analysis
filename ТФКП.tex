...
\begin{document}
	...
	\begin{theorem}
		Пусть функция $f(z)$ определена на области $D$, $z_0 \in D$. Тогда следующие условия эквивалентны:
		\begin{enumerate}
			\item $f(z) = u(x, y) +iv(x, y)$ дифференцируема в точке $z_0$.
			\item Функции $u(x,y)$ и $v(x,y)$ дифференцируемы в точке $(x_0,y_0)$ и удовлетворяют условиям Коши–Римана:
			\[
			\begin{cases}
				u'_x(x_0,y_0) = v'_y(x_0,y_0)\\
	...
	\noindent
	\begin{example}
		Исследовать $f(z)$ на дифференцируемость на $\Cm$: $f(z) = z^2 - 2(\overline z)^2$
	\end{example}
	\begin{proof}
	...
	\[ \text{Пусть } \Um (r, \phi) = u (r \cos \phi, r \sin \phi)\]
	\[ \text{Пусть } \Vm (r, \phi) = v (r \cos \phi, r \sin \phi)\]
	...
	\begin{proof}
		...
		\[\Vm'_r(r, \phi) = v'_m x'_r + u'_y y'_r = v'_x \cos \phi + v'_y \sin \phi\]
		\[\Vm'_{\phi}(r, \phi) = v'_x x'_{\phi} + v'_y y'_{\phi} = v'_x (-r\sin \phi) + v'_y (r\cos \phi)\]
		Зная что $\begin{cases}u'_x = v'_y\\ u'_y = -v'_x\end{cases}$ получим $ \implies r\Um'_r = \Vm'_{\phi} \text{ и } -r\Vm'_r = \Um'_{\phi}$
	\end{proof}
	...
	\end{definition}
	\begin{definition}
		Функция $f(z)$ называется \textbf{аналитической в области $D$}, если она аналитична в $\forall$ точке области D.
	\end{definition}
	\begin{remark}
