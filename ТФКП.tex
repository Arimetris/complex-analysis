...
\begin{document}
	...
	\end{tikzpicture}\\
	Тогда точка $\omega_0 = f(z_0)$ и рассмотрим сложную функцию $\omega(t) = f(z(t))$. и найдем ее производную в точке $t_0$:
	...
	\begin{statement}
		Вещественнозначная функция $u(x, y)$ называется \textbf{гармонической} в области $D$, если $u(x, y) \in C^2(D)$ и удовлетворяет уравнению Лапласа $\Delta u = 0$,  где $\Delta u = \nabla^2 u = u''_{xx} + u''_{yy}$, $(x, y) \in D$. 
	\end{statement}  
	\begin{remark}
		Уравнение в частных производных вида $\Delta u = 0$ называется \textbf{уравнением Лапласа}.
	\end{remark}
	\begin{theorem}
		Если $f(z) \in \mathcal{A}(D) \implies \operatorname{Re} f, \operatorname{Im} f$ --- гармонические в $D$. 
	\end{theorem}
	\begin{proof}
		\[f(z) = \underbrace{u(x,y)}_{\operatorname{Re} f} + i\underbrace{v(x,y)}_{\operatorname{Im}f}\]
		\[\forall (x,y) \in D \implies \begin{cases} u'_x = v'_y \ | \ \frac{\partial}{\partial x} \\ u'_y = -v'_x \ | \ \frac{\partial}{\partial y}\end{cases} \implies u''_{xx} + u''_{yy} = v''_{yx} - v''_{xy} = 0\] 
		или тоже самое для функции $v(x, y)$:
		\[\forall (x,y) \in D \implies \begin{cases} v'_y = u'_x \ | \ \frac{\partial}{\partial y} \\ v'_x = -u'_y \ | \ \frac{\partial}{\partial x}\end{cases} \implies v''_{yy} + v''_{xx} = u''_{xy} - u''_{yx} = 0\]  
	\end{proof}
	\begin{statement}
		Две гармонические в области $D$ функции, связанные условием Коши-Римана называют \textbf{гармонически-сопряженными}.
	\end{statement}
	\vspace{0.25cm}
	\begin{remark}
	...
	Ответом на данный вопрос служит теорема:
	\begin{theorem}
	...
	\begin{proof}
		...
		\[\mathrm{d} v = v'_x \mathrm{d} x + v'_y \mathrm{d}y\]
		\[\mathrm{d} v = \underbrace{-u'_y}_{P} \mathrm{d} x + \underbrace{u'_x}_{Q} \mathrm{d}y\]
		\[(*) \quad v(x,y) = \int_{(x_0, y_0)}^{(x,y)} -u'_y \mathrm{d} x + u'_x \mathrm{d}y + c, \quad c \in \R\]
		Мы должны проверить, что независимость интеграла от пути:
		\[P = -u'_y, Q = u'_x\] 
		Условием является равенство перекрестных производных, то есть:
		\[P_y = Q_x \implies -u''_{yy} = u''_{xx} \implies \Delta u = 0\]
		Как видим, пришли к уравнению Лапласа.
		Тогда аналитическая функция $f(z)$ запишется как:
		\[f(z) = u + iv = u + i \cdot\int_{(x_0, y_0)}^{(x,y)} -u'_y \mathrm{d} x + u'_x \mathrm{d}y + ic, \quad c \in \R\]
